...
UC1 fork of parse_usbpcap.py — only the USB device filter differs. The UC1's
bus/device address is assigned dynamically each time it enumerates, so filtering
by VID/PID (stable across sessions) is preferred over hard-coding an address.
If a capture contains multiple SSL devices, re-capture with UF8 unplugged —
see docs/windows-capture-workflow-uc1.md.
...
def extract_uc1_payloads(pcap_path, display_filter_extra=""):
    """Yield (timestamp, endpoint, direction, payload_bytes) for each UC1 packet."""
    ...
    df = f"usb.idVendor == 0x{SSL_VID:04x} and usb.idProduct == 0x{UC1_PID:04x}"
...
def main():
    ...
    if args.baseline:
        ...
        for _, _, _, payload in extract_uc1_payloads(args.baseline):
            baseline_fps.add(fingerprint(payload))
        print(f"# baseline: {len(baseline_fps)} unique payloads", file=sys.stderr)

    counts = Counter()
    t0 = None
    for ts, ep, direction, payload in extract_uc1_payloads(args.capture):
